...
namespace tei187\ColorTools\Math;

use tei187\ColorTools\Dictionaries\Illuminants\Standard\Tristimulus2;
...
use tei187\ColorTools\Traits\ConvertsBetweenXYandXYZ;
 ...
 * - If illuminant is to be changed (between reference and target), use :php:method:`tei187\ColorTools\Chromaticity\Adaptation\AdaptationMath::adapt()` method on the input values first, and proceed with using the target white point further on.
 * 
 * @see http://www.brucelindbloom.com/
 */
class ModelConversion {
    use ConvertsBetweenXYandXYZ,
        PrimariesLoader;
...
// XYZ
    ...
    public static function XYZ_to_RGB(array $data, $primaries = 'sRGB', ?array $WP_RefTristimulus = Tristimulus2::D65) {
        ...
        if($WP_RefTristimulus !== null) {
            foreach($primaries->getPrimariesXYY() as $values) {
                $primariesXYZ[] = array_values(
                    AdaptationMath::adapt(
                        self::xyY_to_XYZ($values), 
        ...
        $matrix = AdaptationMath::transpose3x3Matrix( AdaptationMath::invert3x3Matrix($primariesXYZ) );
        $xyz_rgb = AdaptationMath::matrixVector($matrix, array_values($data));
...
// RGB
    ...
    public static function RGB_to_XYZ(array $data, $primaries = 'sRGB', $simple = true) {
        ...
        if($simple) {
            return AdaptationMath::matrixVector(AdaptationMath::transpose3x3Matrix($primariesXYZ), $rgb_gamma);
        } else{
            return [
    ...
    public static function _checkWhitePoint($var) {
        if(is_string($var)) {
            ...
            if($check === false) {
                ...
                return constant("\\tei187\\ColorTools\\Dictionaries\\Illuminants\\Standard\\Tristimulus".$angle."::".strtoupper(trim($var[0])));
            }
        } elseif(is_array($var)) {
            $length = count($var); 
            switch($length) {
